...
public class EventesviewController implements Initializable {
    ...
    private Boolean testNom() {
        ...
        if (pat.matcher(catnom.getText()).matches() == false) {
...
//            
     ...
     private Boolean testType() {
        ...
        if (pat.matcher(cattype.getText()).matches() == false) {
            checkType.setImage(new Image("Images/ErreurcheckMark.png"));
            erreur = erreur + ("Veuillez verifier votre nom\n");
            return false;
//            

        } else {
            checkType.setImage(new Image("Images/checkMark.png"));
        }
        return true;
