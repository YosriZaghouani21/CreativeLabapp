...
public class CoachClientController implements Initializable {
    ...
    private void hundelbutton(ActionEvent event) {
        try {
            FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/viewsCoach/FormulaireCoach.fxml"));
    ...
    private void ReserverCoach(ActionEvent event) {
     try {
            FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/viewsCoach/reservationPartieClient.fxml"));
            Parent root1 = (Parent) fxmlLoader.load();
            Stage stage = new Stage();
            stage.initModality(Modality.APPLICATION_MODAL);
            stage.initStyle(StageStyle.UNDECORATED);
